...
int MsgPraser::PraseJson(const string& PostMsg, unordered_map<string,string>& JsonMsg){
    int iRet = SUCCESS;
    do{
        string Msg(PostMsg);
        if (Msg.empty()) {
        ...
        while ((pos = Msg.find_first_of(',')) != Msg.npos){
            ...
            JsonMsg.insert(pair<string,string>(SubJson.substr(begin+1,SubJsonSpiltPos-begin-2),
        ...
        JsonMsg.insert(pair<string, string>(Msg.substr(1,SubJsonSpiltPos-2),
