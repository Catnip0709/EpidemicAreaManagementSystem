...
```
## 18、管理员修改用户隔离状态 QuarantineStatChange
...
## 19、管理员修改用户当前状态 HealthStateChange
...
## 22、根据用户名、栋、户、手机号查找用户信息 viewUserInfo
+ 参数  {"userName":"张三三","buildingID:"",familyID:"",phone:"13344445555"}
string userName   用户名字  
string buildingID 栋号  
string familyID   户号  
string phone      手机号  
四项参数中可以只有其中几项有值，无值的就空字符串  
+ 返回值
```
{
  "result":"0,
...
+ 测试命令  
curl localhost:9779/viewUserInfo -X POST -H "Content-Type:application/json" -d '"userName":"","buildingID":"1","familyID":"2","phone":""'
