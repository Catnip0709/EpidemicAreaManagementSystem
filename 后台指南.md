...
```
+ 测试命令  
curl localhost:9779/epidemicSituation  
...
## 22、管理员查看指定日期的人民上报情况 viewPhysicalCondition
+ 参数 {"date":"2020-05-28"}  
string date   指定的日期  
+ 返回值  
正确返回  
...
## 23、人民获取自己申请的物资的情况 getApplyStatus  
...
## 24、根据用户名、栋、户、手机号查找用户信息 viewUserInfo
...
+ 测试命令  
curl localhost:80/viewUserInfo -X POST -H "Content-Type:application/json" -d '{"userName":"","buildingID":"1","familyID":"","phone":""}'
